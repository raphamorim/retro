ace.define("ace/ext/chromevox",["require","exports","module","ace/editor","ace/config"], (require, exports, module) => {
const cvoxAce = {};
...
const getKeyShortcutString = keyCode => KEY_PREFIX + String.fromCharCode(keyCode);
const isVimMode = () => {
  const keyboardHandler = cvoxAce.editor.keyBinding.getKeyboardHandler();
  return keyboardHandler.$id === 'ace/keyboard/vim';
};
const getCurrentToken = cursor => cvoxAce.editor.getSession().getTokenAt(cursor.row, cursor.column + 1);
const getCurrentLine = cursor => cvoxAce.editor.getSession().getLine(cursor.row);
const onRowChange = currCursor => {
...
const expand = (value, replaceRules) => {
  let newValue = value;
...
const mergeTokens = (tokens, start, end) => {
  const newToken = {};
  newToken.value = '';
...
const speakTokenFlush = token => {
  speakToken_(token, 0);
};
var speakTokenQueue = token => {
  speakToken_(token, 1);
};
var getTokenRule = token => {
  if (!token || !token.type) {
    return;
  }
...
var speakChar = cursor => {
  const line = getCurrentLine(cursor);
  cvox.Api.speak(line[cursor.column], 1);
};
const speakDisplacement = (lastCursor, currCursor) => {
...
const speakCharOrWordOrLine = (lastCursor, currCursor) => {
...
const onColumnChange = (lastCursor, currCursor) => {
...
const isNewAnnotation = annot => {
  const row = annot.row;
  const col = annot.column;
  return !annotTable[row] || !annotTable[row][col];
};
const populateAnnotations = annotations => {
  annotTable = {};
...
const speakAnnot = annot => {
  let annotText = `${annot.type} ${annot.text} on ${rowColToString(annot.row, annot.column)}`;
  annotText = annotText.replace(';', 'semicolon');
  cvox.Api.speak(annotText, 1);
};
const speakAnnotsByRow = row => {
  const annots = annotTable[row];
  for (const col in annots) {
    speakAnnot(annots[col]);
  }
};
var rowColToString = (row, col) => `row ${row + 1} column ${col + 1}`;
const speakCurrRowAndCol = () => {
  cvox.Api.speak(rowColToString(lastCursor.row, lastCursor.column));
};
const speakAllAnnots = () => {
...
const onChangeStatus = (evt, editor) => {
  if (!isVimMode()) {
    return;
  }
