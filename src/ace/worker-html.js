...
ace.define("ace/mode/html/saxparser",["require","exports","module"], (require, exports, module) => {
...
1:[(_dereq_, module, exports) => {
    ...
    class ElementStack {
        ...
        pushHtmlElement(item) {
            this.rootNode = item.node;
            this.push(item);
        }

        pushHeadElement(item) {
            this.headElement = item.node;
            this.push(item);
        }

        pushBodyElement(item) {
        ...
        remove_openElements_until(callback) {
            let finished = false;
            let element;
            while (!finished) {
...
4:[(_dereq_, module, exports) => {
    ...
    class StackItem {
        constructor(namespaceURI, localName, attributes, node) {
        ...
        isSpecial() {
            return this.namespaceURI in SpecialElements &&
                SpecialElements[this.namespaceURI].includes(this.localName);
        }
                ...
                if (data === InputStream.EOF) {
                    ...
                    if (['p', 'P'].includes(data)) {
                        ...
                        var matched = expected.every(expected => {
                            data = buffer.char();
                            return expected.includes(data);
                        });
                        if (matched) {
                            tokenizer.setState(after_doctype_public_keyword_state);
                            return true;
                        }
                    } else if (['s', 'S'].includes(data)) {
    ...
    class TreeBuilder {
        constructor() {
            ...
            modes.inHead.processEOF = function() {
                ...
                if (['title', 'style', 'script'].includes(name)) {
            ...
            modes.inBody.startTagListItem = function(name, attributes) {
                ...
                for (let i = els.length - 1; i >= 0; i--) {
                    ...
                    if (stopName.includes(node.localName)) {
            ...
            modes.inColumnGroup.ignoreEndTagColgroup = () => tree.currentStackItem().localName == 'html';
            ...
            modes.inForeignContent.processStartTag = (name, attributes, selfClosing) => {
                if (['b', 'big', 'blockquote', 'body', 'br', 'center', 'code', 'dd', 'div', 'dl', 'dt', 'em', 'embed', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'head', 'hr', 'i', 'img', 'li', 'listing', 'menu', 'meta', 'nobr', 'ol', 'p', 'pre', 'ruby', 's', 'small', 'span', 'strong', 'strike', 'sub', 'sup', 'table', 'tt', 'u', 'ul', 'var'].includes(name)
                        || (name == 'font' && attributes.some(attr => ['color', 'face', 'size'].includes(attr.nodeName)))) {
            ...
            modes.inHeadNoscript.startTagBasefontBgsoundLinkMetaNoframesStyle = (name, attributes) => {
                modes.inHead.processStartTag(name, attributes);
            };
        ...
        processToken(token) {
            ...
            switch(token.type) {
            case 'Characters':
                const buffer = new CharacterBuffer(token.data);
        ...
        generateImpliedEndTags(exclude) {
            ...
            if (['dd', 'dt', 'li', 'option', 'optgroup', 'p', 'rp', 'rt'].includes(name) && name != exclude) {
                this.popElement();
...
10:[(_dereq_, module, exports) => {
    ...
    class DTD {
        constructor(name, publicIdentifier, systemIdentifier) {
            ParentNode.call(this);
            this.name = name;
...
11:[(_dereq_, module, exports) => {
    class TreeParser {
        constructor(contentHandler, lexicalHandler) {
            this.contentHandler;
            this.lexicalHandler;
        ...
        characters(ch, start, length, locator) {
        ...
        endElement(uri, localName, qName, locator) {
            this.locatorDelegate = locator;
            this.contentHandler.endElement(uri, localName, qName);
        }

        endPrefixMapping(prefix, locator) {
            this.locatorDelegate = locator;
            this.contentHandler.endPrefixMapping(prefix);
        }

        ignorableWhitespace(ch, start, length, locator) {
            this.locatorDelegate = locator;
            this.contentHandler.ignorableWhitespace(ch, start, length);
        }

        processingInstruction(target, data, locator) {
            this.locatorDelegate = locator;
            this.contentHandler.processingInstruction(target, data);
        }

        skippedEntity(name, locator) {
            this.locatorDelegate = locator;
            this.contentHandler.skippedEntity(name);
        }

        startDocument(locator) {
            this.locatorDelegate = locator;
            this.contentHandler.startDocument();
        }

        startElement(uri, localName, qName, atts, locator) {
            this.locatorDelegate = locator;
            this.contentHandler.startElement(uri, localName, qName, atts);
        }

        startPrefixMapping(prefix, uri, locator) {
            this.locatorDelegate = locator;
            this.contentHandler.startPrefixMapping(prefix, uri);
        }

        comment(ch, start, length, locator) {
            this.locatorDelegate = locator;
        ...
        startDTD(name, publicId, systemId, locator) {
            this.locatorDelegate = locator;
            this.lexicalHandler.startDTD(name, publicId, systemId);
        }

        startEntity(name, locator) {
            this.locatorDelegate = locator;
...
ace.define("ace/mode/html_worker",["require","exports","module","ace/lib/oop","ace/lib/lang","ace/worker/mirror","ace/mode/html/saxparser"], (require, exports, module) => {
    const oop = require("../lib/oop");
    const lang = require("../lib/lang");
